...
public class AdminAddUserController {
	...
	@GetMapping
	@PreAuthorize("hasRole('ADMIN')")
	public String showRegistrationForm(ModelMap map, Authentication authentication) {
		List<String> roles = moduleRunFirst.getRole(authentication);
		moduleRunFirst.headerFooter(authentication, map, roles);
		return "user_admin/admin/adminAddUser";
	}
	...
	public String registerUserAccount(@ModelAttribute("person") @Valid AdminDto userDto, BindingResult result,
		...
		if (moduleRunFirst.isAdmin(moduleRunFirst.getRole(authentication))) {
