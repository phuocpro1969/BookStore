...
public class DataNewService implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		if (categoryRepository.findById((long) 1) == null) {
			categoryRepository.save(new Category("updatingCategory", moduleRunFirst.getTimeNow(),
					moduleRunFirst.getTimeNow(), (long) 1, (long) 1));
		}

		if (publisherRepository.findById((long) 1) == null) {
			publisherRepository.save(new Publishers("updatingPublisher", moduleRunFirst.getTimeNow(),
					moduleRunFirst.getTimeNow(), (long) 1, (long) 1));
		}
		...
		if (userRepository.findByUsername("admin") == null) {
			...
			folder = new File("src/main/resources/static/images/", "personImage");
			if (folder.exists() && folder.isDirectory())
				for (File f : folder.listFiles())
					f.delete();
			if (!folder.exists())
				folder.mkdir();
			File storePictureFile = uploadPathService.getFilePath("noImage.jpg", "personImage");
			try {
			...
			String modifiedFileName = "1_" + originalFileName;
			storePictureFile = uploadPathService.getFilePath(modifiedFileName, "personImage");
			if (storePictureFile != null) {
				try {
					byte[] bFile = Files.readAllBytes(Paths.get("src/main/resources/static/images/admin.jpg"));
					FileUtils.writeByteArrayToFile(storePictureFile, bFile);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			admin.setPicture(modifiedFileName);
